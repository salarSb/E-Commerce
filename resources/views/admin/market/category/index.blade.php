                                ...
                                <td>
                                    <label>
                                        <input id="{{ $productCategory->id }}"
                                               ...
                                               data-url="{{ route('admin.market.category.status', $productCategory->slug) }}"
                                               @if($productCategory->status === 1) checked @endif>
                                    </label>
                                </td>
                                <td>
                                    <label>
                                        <input id="{{ $productCategory->id }}-show-in-menu"
                                               onchange="changeShowInMenu({{ $productCategory->id }})" type="checkbox"
                                               data-url="{{ route('admin.market.category.show-in-menu', $productCategory->slug) }}"
                                               @if($productCategory->show_in_menu === 1) checked @endif>
                                    </label>
                                </td>
                                <td class="width-16-rem text-left">
                                    <a href="{{ route('admin.market.category.edit', $productCategory->slug) }}"
                                       class="btn btn-sm btn-primary">
                                        <i class="fa fa-edit ml-1"></i>
                                        ویرایش
                                    </a>
                                    <form action="{{ route('admin.market.category.destroy',$productCategory->slug) }}"
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status) {
                        if (response.checked) {
                            element.prop('checked', true);
                            successToast('دسته بندی در منو به نمایش در می آید')
                        } else {
                            element.prop('checked', false);
                            successToast('دسته بندی در منو به نمایش در نمی آید')
                        }
                    } else {
                        element.prop('checked', elementValue);
                        errorToast('هنگام ویرایش مشکلی رخ داده است')
